...
inputs = np.zeros((int(numPatches), int(config.pat_size), int(config.pat_size), 3), dtype=np.uint8)
...
for i in range(len(imgArray)):
    ...
    for s in range(len(scales)):
        ...
        for x in range(0 + config.step, im_h - config.pat_size, config.stride):
            for y in range(0 + config.step, im_w - config.pat_size, config.stride):
                inputs[count, :, :, :] = img_s[x:x + config.pat_size, y:y + config.pat_size, :]
                count += 1
